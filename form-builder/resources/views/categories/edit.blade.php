<x-app-layout>
    ...
    <div class="py-12">
        ...
        <form class="max-w-md mx-auto" action="{{route('categories.update',$category->id)}}" method="POST" >
            ...
            <div class="relative z-0 w-full mb-5 group">
                <label for="organization_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Crganization</label>
                <select id="organization_id" name="organization_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                  ...
                  @foreach ($organization as $item)
                    <option {{$item->id==$category->organization_id ? 'selected' : '' }} value="{{$item->id}}">{{$item->name}}</option>
                  @endforeach
                </select>
